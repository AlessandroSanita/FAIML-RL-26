...
def main():
    ...
    for baseline in baselines:
        ...
        rewards[baseline], time_taken[baseline], time_update[baseline] = train_model(baseline=baseline, num_episodes=50000)
    ...
    for baseline in baselines:
        plt.plot(rewards[baseline], label=f'Baseline: {baseline}', alpha=0.7)
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    ...
    plt.figure(figsize=(12, 8))
    for baseline in baselines:
        plt.plot(time_taken[baseline], label=f'Baseline: {baseline}', alpha=0.7)
    plt.xlabel('Episode')
    ...
    # visualizing time update
    plt.figure(figsize=(12, 8))
    for baseline in baselines:
        plt.plot(time_update[baseline], label=f'Baseline: {baseline}', alpha=0.7)
    plt.xlabel('Episode')
